...
public class AudioFilePlayer extends Thread {
	...
	public void run() {
		try {
			...
			System.err.println("InterruptedException in the AudioFilePlayer? Preposterous!");
			e.printStackTrace();
		}
		play();
	}
	...
	private boolean shouldKeepPlaying = true;
	...
	private void rawplay(AudioFormat targetFormat, AudioInputStream din) {
		...
		try {
			...
			if (line != null) {
				...
				int nBytesWritten = 0;
				while (nBytesRead != -1 && shouldKeepPlaying) {
				...
				if (shouldKeepPlaying) {
					for (EndOfSongListener listener : listeners) {
						EndOfSongEvent eose = new EndOfSongEvent(fileName, new GregorianCalendar());
